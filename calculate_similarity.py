...
def load_assembly_data(data):
    ...
    for i in range(0, len(data) -1, 2):
        data_pairs.append((data[i].strip(), data[i + 1].strip()))
    return data_pairs


def get_bert_output(data, model, tokenizer):
    batch_input = load_assembly_data(data)
    batch_input = pd.DataFrame(batch_input)
    test_dataset = BERTDataset(batch_input, tokenizer, seq_len=16, train=False)
